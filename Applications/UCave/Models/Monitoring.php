...
class Monitoring {
	...
	private function Ping() {
		...
		if (json_last_error() !== JSON_ERROR_NONE) {
			$this->error = function_exists('json_last_error_msg') ? json_last_error_msg() : 'JSON parsing failed';
	...
	private function getStats($ip, $port=25565){
		$result = [
			'status' => false,
			'online' => 0,
			'slots' => 0,
		...
		$stats = $this->GetStatus($ip, $port)->Response();

		if($stats['online']){
			$result['status'] = true;
			$result['online'] = intval($stats['players']);
			$result['slots'] = intval($stats['max_players']);
		}
	...
	public function getUpdatedData(){
		...
		if($time > ($config['cache']['updated']+$config['cache']['time'])){
			$stats = $this->getStats($config['ip']);

			$config['cache']['status'] = $stats['status'];
			$config['cache']['online'] = $stats['online'];
			$config['cache']['slots'] = $stats['slots'];
			$config['cache']['updated'] = $time;

			if($stats['online']>$config['cache']['record']){
				$config['cache']['record'] = $stats['online'];
			}

			if(date("d", $config['cache']['updated'])!=date("d")){
				$config['cache']['today'] = $stats['online'];
			}

			$data = "<?php".PHP_EOL.PHP_EOL;
			$data .= "return ".var_export($config, true).";".PHP_EOL.PHP_EOL;
			$data .= "?>";

			$root = dirname(__DIR__);

			file_put_contents("{$root}/Config.php", $data, LOCK_EX);
		}

		return [
			'status' => $config['cache']['status'],
			'online' => $config['cache']['online'],
			'slots' => $config['cache']['slots'],
			'record' => $config['cache']['record']
		];
	}
}
