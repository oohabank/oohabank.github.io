...
class Listing {
	...
	public function getLimit() : int {
		if(!is_int($this->limit)){
			$this->limit = Servers::PAGINATION_LIMIT;
		}
	...
	public function getCount() : int {
		...
		$select = Database::prepare("SELECT COUNT(1) FROM `servers` WHERE 1 {$and}");
	...
	public function get() : array {
		...
		$select = Database::prepare("SELECT `s`.* FROM `servers` AS `s` WHERE 1 {$and} {$other}", $prepare);
